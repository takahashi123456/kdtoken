...
def start():
    ...
    # 本番↓
    scheduler.add_job(date_farst_execute, 'cron', hour=9, minute=0, max_instances=1)
    scheduler.add_job(id_update, 'cron', year=2022, month=12, day=19, args=[0], id='schedule_predict', max_instances=5) # 変更前提 基本実行されない
    scheduler.add_job(day_schedule_time_apdate, 'cron', hour=9, minute=10, max_instances=1)
    ...
    # scheduler.add_job(score_schedule_execute, 'cron', minute=24, args=[202206050811], max_instances=1)
